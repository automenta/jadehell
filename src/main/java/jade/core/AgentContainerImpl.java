...
/**
 ...
 @version $Date: 2014-07-17 12:23:28 +0200 (gio, 17 lug 2014) $ $Revision: 6718 $
...
class AgentContainerImpl implements AgentContainer, AgentToolkit {
	...
	throws NameClashException, IMTPException, NotFoundException, JADESecurityException {
		...
		GenericCommand cmd = new GenericCommand(AgentManagementSlice.INFORM_CREATED, AgentManagementSlice.NAME, null);
	...
	public void handleChangedAgentState(AID agentID, int oldState, int newState) {
		...
		GenericCommand cmd = new GenericCommand(AgentManagementSlice.INFORM_STATE_CHANGED, AgentManagementSlice.NAME, null);
		cmd.addParam(agentID);
		cmd.addParam(from);
		cmd.addParam(to);
		// No security check is meaningful on this action --> don't even set the Credentials
	...
	public void handleEnd(AID agentID) {
		GenericCommand cmd = new GenericCommand(AgentManagementSlice.INFORM_KILLED, AgentManagementSlice.NAME, null);
		cmd.addParam(agentID);
